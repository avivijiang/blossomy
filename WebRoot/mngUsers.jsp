...
if(list==null){  
    ...
    list = service.listUser("", start, end);  
}
%>
...
<script type="text/javascript">  
    function searchUser(){  
        ...
        th.action="<%=path%>/servlet/UserAction?action_flag=search";  
        th.submit();  
    }  
      
    function first(){  
          
        window.location.href = "<%=path%>/servlet/UserAction?action_flag=search&pageNum=1";  
          
    }  
    function next(){  
          
        window.location.href = "<%=path%>/servlet/UserAction?action_flag=search&pageNum=<%=dividePage.getCurrentPage()+1%>";       
    ...
    function end(){  
        ...
        window.location.href = "<%=path%>/servlet/UserAction?action_flag=search&pageNum=<%=dividePage.getPageCount()%>";  
              
    }  
      
    function changePage(currentPage){  
      
        window.location.href = "<%=path%>/servlet/UserAction?action_flag=search&pageNum="+currentPage;  
    ...
    function view(){  
        ...
        var th = document.form1;  
        th.action="<%=path%>/servlet/UserAction?action_flag=view&proid="+getSelectedValue();  
        th.submit();          
  ...
  <ul class="animate">
    <li class="animate" onclick="javascript:location.href='<%=path%>/addhomepage.jsp'">热门推荐上新<i class="fa fa-code float-right"></i></li>
		...
		<div class="panel panel-default"
			...
			<div class="panel-heading" style="height:50px">用户管理</div>
			...
			<div style="margin:20 20 0 40;float:left;display:inline-block">
				<button type="button" class="btn btn-success btn-sm"  style="margin-right: 30px"onclick="javascript:del();">删除</button>
					...
					<form class="form-inline" name="form2" method="post" role="form" >
                        ...
                        <label class="control-label" for="inputSuccess2"  style="padding-right:25px">用户查询</label>
                        <input type="text" class="form-control" name="username"   id="inputSuccess2" 	value="<%=userName != null ? userName : ""%>">
                        <span class="glyphicon glyphicon-search"  style="left:-25px" onclick="searchUser()"></span>
                          </div>
                        </form>
			</div>
